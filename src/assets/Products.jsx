...
import { CustomLoading, Loading } from "./components/Loading";
...
const Products = () => {
  ...
  const filterFunc = (e) => {
    ...
    if (itemCatName === "all") {
      ...
      const categoryItems = filterProducts.filter((item) =>
        item.itemCategory.toLowerCase().includes(itemCatName.toLowerCase())
      );
          ...
          <div className="lg:flex lg:justify-center ">
            <div className="relative   lg:w-[50%]  ">
              <input
                ...
                className="w-full border-2 rounded-full h-[2.5rem] pl-4 border-indigo-500 outline-2 placeholder:text-gray-600 outline-indigo-700 "
              />
              <FaSearch
                size={20}
            ...
            <>
              {products.length > 0 ? (
                <div className="lg:flex lg:flex-wrap lg:justify-center lg:gap-4 ">
                  {products.map((item) => (
                    <Link
                      to={`/products/product_over_view/${item._id}`}
                      key={item._id}
                      className="lg:w-[35%] flex border hover:shadow-gray-400 relative items-start gap-3 mb-3 shadow-md rounded shadow-gray-300 p-2"
                    >
                ...
                <div className="flex justify-center items-center w-full h-[50vh]">
                  <h5 className="text-[1.2rem] font-semibold">No Products</h5>
                </div>
              )}
            </>
          )}
